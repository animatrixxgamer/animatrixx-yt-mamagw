...
> ⚠️ **Historical — describes the removed `src/` app.** These notes were written
> for the aiogram + FastAPI rewrite (`python -m src.main`, `from src.bot.buttons
> import Btn`), which has been deleted along with its `Dockerfile`. `bot.py` has
> since inherited most of these features; the code the snippets below refer to is
> in `legacy-aiogram-app.zip`.
