...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS users
                 (user_id INTEGER PRIMARY KEY, username TEXT, status TEXT DEFAULT 'free',
                  ...
                  credit_source INTEGER DEFAULT 1)''')
    ...
    _ensure_columns(conn, "users", {
        ...
        "credit_source": "INTEGER DEFAULT 1",
        "desc_template": "TEXT",
        "creator_line_template": "TEXT",
        "source_line_template": "TEXT",
        "shorts_tag": "TEXT",
    })
    c.execute('''CREATE TABLE IF NOT EXISTS force_join_channels
                 (id INTEGER PRIMARY KEY AUTOINCREMENT, chat_id TEXT UNIQUE,
                  title TEXT, username TEXT, invite_link TEXT, added_at TEXT)''')
    # Databases created before invite_link existed break check_force_join()
    # with sqlite3.OperationalError, which silently killed /start entirely
    # (no welcome AND no join gate — users could use the bot without joining).
    _ensure_columns(conn, "force_join_channels", {
        "invite_link": "TEXT",
    })
    ...
    _ensure_columns(conn, "tiktok_sources", {
        "credit_source": "INTEGER",
    })
...
def user_credit_source(uid: int) -> int:
    """1 = append a 'Source: <link>' line to mirrored descriptions (default), 0 = leave it out."""
    row = db_query('SELECT credit_source FROM users WHERE user_id = ?', (uid,), fetch=True)
    return 0 if (row and row[0][0] == 0) else 1


def save_user_credit_source(uid: int, value: int) -> None:
    """Set the user-wide source-credit preference (1 = on, 0 = off)."""
    db_query('UPDATE users SET credit_source = ? WHERE user_id = ?', (1 if value else 0, uid))


def source_credit_source(src: Optional[tuple]) -> int:
    """Resolve the credit preference for a tiktok_sources row.

    A source row with credit_source NULL follows the user default; an explicit
    0/1 on the row overrides it. Rows from pre-migration databases (missing the
    column) also fall back to the user default.
    """
    if src is None:
        return 1
    value = src[14] if len(src) > 14 else None
    if value is None:
        return user_credit_source(int(src[1]))
    return 1 if value else 0


def append_source_credit(base_text: str, url: str, credit: int, uid: int = 0) -> str:
    """Append the user's configured source line only when they want it."""
    line = user_source_line_template(uid) if uid else DEFAULT_SOURCE_LINE
    if credit and url and line:
        rendered = render_description_template(line, source_url=url)
        return f"{base_text}\n\n{rendered}" if rendered else base_text
    return base_text


# ── User-configurable text (templates appended to descriptions/titles) ────
#
# Every line the bot used to hard-code lives in one of these templates now.
# Available placeholders: {title}, {creator}, {source_url} — {source_url} is
# only filled for link imports / TikTok auto-posts; elsewhere it renders empty.

DEFAULT_CREATOR_LINE = "Original creator: {creator}"
DEFAULT_SOURCE_LINE = "Source: {source_url}"
DEFAULT_SHORTS_TAG = "shorts"


def user_desc_template(uid: int) -> str:
    """Custom block prepended to imported descriptions ('' = disabled)."""
    row = db_query('SELECT desc_template FROM users WHERE user_id = ?', (uid,), fetch=True)
    value = (row[0][0] if row and row[0][0] else "").strip()
    return "" if value.lower() == "off" else value


def save_user_desc_template(uid: int, template: str) -> None:
    """Save the custom description template; '' disables it."""
    db_query('UPDATE users SET desc_template = ? WHERE user_id = ?',
             (template.strip()[:4800], uid))


def user_creator_line_template(uid: int) -> str:
    """How to credit the original creator; '' or 'off' = no creator line."""
    row = db_query('SELECT creator_line_template FROM users WHERE user_id = ?', (uid,), fetch=True)
    value = row[0][0] if row and row[0][0] is not None else None
    if value is None:
        value = DEFAULT_CREATOR_LINE
    value = value.strip()
    return "" if value.lower() == "off" else value


def save_user_creator_line_template(uid: int, template: str) -> None:
    """Save the creator-credit template; 'off' or '' disables the line."""
    template = template.strip()[:600]
    db_query('UPDATE users SET creator_line_template = ? WHERE user_id = ?', (template, uid))


def user_source_line_template(uid: int) -> str:
    """How the source link is shown; '' or 'off' = no source line.

    The legacy on/off preference (credit_source) is honoured: when it is off
    and no explicit template was ever set, the line stays hidden.
    """
    row = db_query('SELECT source_line_template FROM users WHERE user_id = ?', (uid,), fetch=True)
    value = row[0][0] if row and row[0][0] is not None else None
    if value is None:
        return DEFAULT_SOURCE_LINE if user_credit_source(uid) else ""
    value = value.strip()
    return "" if value.lower() == "off" else value


def save_user_source_line_template(uid: int, template: str) -> None:
    """Save the source-link template; 'off' or '' disables the line."""
    template = template.strip()[:600]
    db_query('UPDATE users SET source_line_template = ? WHERE user_id = ?', (template, uid))
    # The template now fully replaces the legacy on/off preference.
    save_user_credit_source(uid, 0 if not template or template.lower() == "off" else 1)


def user_shorts_tag(uid: int) -> str:
    """Tag auto-added to Short uploads ('' = nothing added)."""
    row = db_query('SELECT shorts_tag FROM users WHERE user_id = ?', (uid,), fetch=True)
    value = row[0][0] if row and row[0][0] is not None else None
    if value is None:
        return DEFAULT_SHORTS_TAG  # legacy default for untouched users
    value = value.strip().lstrip("#")
    return "" if value.lower() == "off" else value


def save_user_shorts_tag(uid: int, tag: str) -> None:
    """Save the Shorts auto-tag; '' means no tag is added."""
    db_query('UPDATE users SET shorts_tag = ? WHERE user_id = ?',
             (tag.strip().lstrip("#")[:100], uid))


def render_description_template(template: str, *, title: str = "", creator: str = "",
                                source_url: str = "") -> str:
    """Fill {title}, {creator} and {source_url} placeholders in a template."""
    values = {
...
def build_import_description(uid: int, title: str, creator: str, source_url: str,
                             mark_source: bool = False,
                             credit: Optional[int] = None) -> str:
    """Description for link imports / TikTok auto-posts, fully user-shaped.

    Layers, in order: the user's custom description template, the creator
    line, the source line — each of which the user can edit or turn off.
    With mark_source=True the source line is stored inside an invisible
    marked block so the job card's strip/restore toggle can remove and
    re-add it losslessly (the marker is removed before uploading).
    credit=1/0 overrides the user-wide source-line toggle (used for the
    per-source setting on TikTok sources); None follows the user default.
    """
    parts: List[str] = []
    custom = render_description_template(user_desc_template(uid),
                                         title=title, creator=creator,
                                         source_url=source_url)
    if custom:
        # A custom template owns the whole top block — include {title} there
        # if the title should appear.
        parts.append(custom)
    elif title:
        parts.append(title)

    creator_line = user_creator_line_template(uid)
    if creator_line and (creator or "{creator}" not in creator_line):
        parts.append(render_description_template(creator_line,
                                                 title=title, creator=creator,
                                                 source_url=source_url))

    source_line = user_source_line_template(uid)
    if credit == 1 and not source_line:
        source_line = DEFAULT_SOURCE_LINE  # per-source override wins over user off
    elif credit == 0:
        source_line = ""
    if source_line and (source_url or "{source_url}" not in source_line):
        parts.append(render_description_template(source_line,
                                                 title=title, creator=creator,
                                                 source_url=source_url))

    body = "\n".join(parts).strip() or (title or "")
    if mark_source and source_line and parts and parts[-1] == \
            render_description_template(source_line, title=title, creator=creator,
                                        source_url=source_url):
        # Mark the rendered source line so the job card can strip/restore it.
        # The block holds only what the user sees; restore re-renders from the
        # job's source_url, so toggling never duplicates lines.
        head = "\n".join(parts[:-1]).strip()
        marked = f"{SOURCE_SUFFIX_MARK}{parts[-1]}"
        return f"{head}\n\n{marked}" if head else marked
    return body


SOURCE_SUFFIX_MARK = "\u200b\U0001f517\u200b"  # zero-width space + 🔗 marker


def split_source_suffix(description: str) -> tuple:
    """Split a stored description into (base_text, source_url_or_'').

    Newly created jobs carry the source inside a marked suffix block, so the
    strip/restore toggle on the job card can remove and re-add it losslessly.
    Older jobs (a plain 'Source: <url>' line or a bare URL) are handled too.
    """
    text = str(description or "")
    mark = SOURCE_SUFFIX_MARK
    if mark in text:
    ...
    for line in reversed([l for l in text.splitlines() if l.strip()]):
        ...
        if re.match(r"^https?://\S+$", candidate):
            base = text[:text.rfind(line)].rstrip("\n")
            return base, candidate
    return text, ""


def build_stored_description(base_text: str, source_line: str) -> str:
    """Store base text plus a marked source block for later strip/restore."""
    base_text = (base_text or "").strip()
    source_line = (source_line or "").strip()
    if not source_line:
        return base_text
    return f"{base_text}\n\n{SOURCE_SUFFIX_MARK}{source_line}"


def job_source_line_enabled(job_id: int) -> bool:
    """True when the job still carries its source line (shown on the job card)."""
    job = get_job(job_id)
    if not job:
        return False
    _base, stored = split_source_suffix(job_field(job, "description") or "")
    return bool(stored)


def toggle_job_source_line(job_id: int) -> str:
    """Strip or restore the source line on a queued job. Returns the new state."""
    job = get_job(job_id)
    if not job:
        return "missing"
    base, stored = split_source_suffix(job_field(job, "description") or "")
    if stored:
        db_query('UPDATE upload_jobs SET description = ? WHERE job_id = ?', (base, job_id))
        return "off"
    template = user_source_line_template(int(job_field(job, "user_id")))
    url = job_field(job, "source_url") or ""
    if not template or not url:
        return "unavailable"
    line = render_description_template(template, title=job_field(job, "title") or "",
                                       creator="", source_url=url)
    db_query('UPDATE upload_jobs SET description = ? WHERE job_id = ?',
             (build_stored_description(base, line), job_id))
    return "on"
...
def force_join_channels() -> List[tuple]:
    """All channels/groups users must join. Returns (chat_id, title, username, invite_link)."""
    return db_query('SELECT chat_id, title, username, invite_link FROM force_join_channels ORDER BY id',
                    fetch=True) or []


def add_force_join_channel(chat_id: str, title: str = "", username: str = "",
                          invite_link: str = "") -> bool:
    """Register a channel/group for force-join."""
    try:
        db_query('INSERT OR IGNORE INTO force_join_channels '
                 '(chat_id, title, username, invite_link, added_at) '
                 'VALUES (?, ?, ?, ?, ?)',
...
def check_force_join(uid: int, chat_id: int) -> bool:
    ...
    if not required:
        return True
    not_joined = []
    for chat_id_str, title, username, invite_link in required:
        can_verify = False
        # Try hard check if bot is admin in the channel
        try:
            member = bot.get_chat_member(chat_id_str, uid)
            if member.status in ("left", "kicked"):
                not_joined.append((chat_id_str, title, username, invite_link, True))
                can_verify = True
            elif member.status in ("member", "administrator", "creator"):
                can_verify = True
                # User is a member — skip
        except Exception:
            pass  # Can't verify — fall through to trust-based
        if not can_verify:
            # Bot can't check membership — trust-based (user taps I Joined)
            not_joined.append((chat_id_str, title, username, invite_link, False))
    if not not_joined:
        return True
    ...
    verified_count = 0
    trust_count = 0
    for chat_id_str, title, username, invite_link, can_verify in not_joined:
        # Determine the join URL
        join_url = ""
        if invite_link:
            join_url = invite_link
        elif username:
            join_url = f"https://t.me/{username}"
        elif chat_id_str and str(chat_id_str).startswith("-"):
            # Private channel — use invite link or fallback
            join_url = f"https://t.me/c/{str(chat_id_str).replace('-100', '')}"
        else:
            join_url = f"https://t.me/{chat_id_str}"
        label = title or username or f"Channel {chat_id_str}"
        kb.add(types.InlineKeyboardButton(f"📺 Join {label}", url=join_url))
        if can_verify:
            verified_count += 1
        else:
            trust_count += 1
    ...
    names = "\n".join(f"  • {title or username or cid}" for cid, title, username, _, _ in not_joined)
    verify_note = ""
    if trust_count > 0 and verified_count == 0:
        verify_note = ("\n\n⚠️ <i>I can't verify membership automatically — "
                      "tap I joined after you subscribe.</i>")
    elif trust_count > 0:
        verify_note = ("\n\n<i>Some channels can be verified automatically, "
                      "others trust your tap.</i>")
    safe_send(chat_id,
...
def add_source(uid: int, handle: str, profile_url: str, channel_id: str,
               ...
               stagger_minutes: int = 0, title_template: str = "{title}",
               credit_source: Optional[int] = None) -> int:
    return db_insert(
        ...
        'credit_source, created_at) VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?)',
        (uid, handle, profile_url, channel_id, privacy, max_per_run,
         stagger_minutes, title_template, credit_source,
         datetime.utcnow().isoformat()))


def set_source_credit(source_id: int, uid: int, value: Optional[int]) -> None:
    """Set a per-source credit override; None clears it back to the user default."""
    db_query('UPDATE tiktok_sources SET credit_source = ? WHERE source_id = ? AND user_id = ?',
             (value, source_id, uid))
...
def run_job_upload(job_id: int, quiet: bool = False) -> bool:
    ...
    with UPLOAD_LOCK:
        ...
        try:
            ...
            # The invisible marker only marks the source line for the
            # strip/restore toggle; YouTube must never see it.
            stored_desc = (job_field(job, "description") or "").replace(SOURCE_SUFFIX_MARK, "")
            metadata = {
                "title": title,
...
def poll_source(source_id: int, announce: bool = True) -> int:
    ...
    cells = dict(zip(
        ["source_id", "user_id", "handle", "profile_url", "channel_id", "privacy_status",
         ...
         "last_check", "last_status", "created_at", "credit_source"], raw))
    ...
    for video in videos:
        ...
        # Apply the per-source title template (defaults to a plain {title}).
        title = render_description_template(str(cells.get("title_template") or "{title}"),
                                            title=title, creator=handle, source_url=url)[:100]
        publish_at = None
        if stagger:
        ...
        tk_tags = source_hashtags or "tiktok"
        auto_tag = user_shorts_tag(uid)
        if auto_tag:
            tk_tags = f"{tk_tags},{auto_tag}"
        tk_desc = build_import_description(uid, title, handle, url, mark_source=True,
                                           credit=source_credit_source(raw))
...
def upload_hub_text(uid: int) -> str:
    ...
    lines += [
        ...
        "",
        "📹 <b>YouTube Short</b> — Under 60s, vertical, auto-tagged as a Short",
...
def job_actions_kb(job_id: int, status: str = "pending", user_id: Optional[int] = None) -> types.InlineKeyboardMarkup:
    ...
    if status != "completed":
        kb.add(Btn(f"🔗 Source line: {'on' if job_source_line_enabled(job_id) else 'off'}",
                   callback_data=f"job_srcline:{job_id}", style="primary"))
...
def tk_kb(uid: int) -> types.InlineKeyboardMarkup:
    ...
    kb.add(Btn(f"✍️  Description & credit: {'on' if user_credit_source(uid) else 'off'}",
               callback_data="tk_credit", style="primary"))
    kb.add(Btn(f"{G['back']}  Main Menu", callback_data="main_menu", style="danger"))
    return kb


def tk_credit_kb(uid: int) -> types.InlineKeyboardMarkup:
    """Keyboard for the Description & credit screen."""
    credit = user_credit_source(uid)
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(Btn(f"🔗 Source line: {'on' if credit else 'off'}",
               callback_data="tk_credit_toggle", style="success" if credit else "danger"))
    kb.add(Btn("✍️  Description template", callback_data="tk_edit_desctpl", style="primary"))
    kb.add(Btn("👤  Creator line", callback_data="tk_edit_creator", style="primary"))
    kb.add(Btn("🔗  Source line format", callback_data="tk_edit_srcline", style="primary"))
    kb.add(Btn("#️⃣  Shorts auto-tag", callback_data="tk_edit_shortstag", style="primary"))
    kb.add(Btn(f"{G['back']}  Back", callback_data="tk_menu", style="primary"))
    return kb


def tk_credit_text(uid: int) -> str:
    """Body of the Description & credit screen."""
    credit = user_credit_source(uid)
    desc_tpl = user_desc_template(uid)
    creator_tpl = user_creator_line_template(uid)
    source_tpl = user_source_line_template(uid)
    shorts_tpl = user_shorts_tag(uid)

    def fmt(value: str) -> str:
        return f"<code>{escape(value[:120])}</code>" if value else "<i>off</i>"

    sample = build_import_description(
        uid, "My video title", "@somecreator", "https://tiktok.com/@somecreator/video/1",
        mark_source=True)
    sample_preview = sample.replace(SOURCE_SUFFIX_MARK, "").replace("\n", " ⏎ ")
    return ("✍️ <b>Description &amp; credit</b>\n\n"
            f"These lines get appended to descriptions of imported videos "
            f"(TikTok auto-posts and link imports). Every one is yours to edit:\n\n"
            f"🔗 Source line: <b>{'on' if credit else 'off'}</b>\n"
            f"   Format: {fmt(source_tpl)}\n"
            f"👤 Creator line: {fmt(creator_tpl)}\n"
            f"✍️ Description template: {fmt(desc_tpl)}\n"
            f"#️⃣ Shorts auto-tag: {fmt('#' + shorts_tpl if shorts_tpl else '')}\n\n"
            "Placeholders: <code>{title}</code> <code>{creator}</code> "
            "<code>{source_url}</code>\n"
            "Send <code>off</code> as a value to disable that line.\n\n"
            f"<b>Preview:</b>\n<i>{escape(sample_preview[:400])}</i>")
...
def tk_source_kb(source_id: int, enabled: bool) -> types.InlineKeyboardMarkup:
    ...
    kb.add(
        Btn(f"{G['world']}  Source line", callback_data=f"tk_setcred:{source_id}", style="primary"),
        Btn(f"{G['plus']}  Batch size", callback_data=f"tk_setmax:{source_id}", style="primary"),
    )
    kb.add(Btn("📝  Title template", callback_data=f"tk_settitle:{source_id}", style="primary"))
    kb.add(
        Btn(f"{G['clock']}  Interval", callback_data=f"tk_setint:{source_id}", style="primary"),
        Btn(f"{G['trash']}  Remove source", callback_data=f"tk_del:{source_id}", style="danger"),
    )
    kb.add(Btn(f"{G['back']}  Back", callback_data="tk_menu", style="primary"))
    return kb
...
def cmd_start(message):
    ...
    # Force-join gate: users must join the required channels before the bot
    # does anything for them — /start included. Owners/admins pass freely.
    if get_user_status(uid) in ("free", "premium") and \
            not check_force_join(uid, message.chat.id):
        return
...
def prompt_for_short(chat_id: int, uid: int) -> None:
    ...
    safe_send(chat_id,
              ...
              "and I will add your Shorts tag automatically\n"
              "(set it in TikTok → Description &amp; credit).\n\n"
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("job_srcline:"))
def cb_job_srcline(call):
    """Show the strip/restore panel for a job's source line."""
    uid = call.from_user.id
    job_id = int(call.data.split(":", 1)[1])
    job = get_job(job_id, uid)
    if not job:
    ...
    safe_answer(call)
    desc = job_field(job, "description") or ""
    base, stored = split_source_suffix(desc)
    state = "on" if stored else "off"
    can_restore = bool(job_field(job, "source_url") and user_source_line_template(uid))
    kb = types.InlineKeyboardMarkup(row_width=1)
    if stored:
        kb.add(Btn("🚫 Strip the source line", callback_data=f"job_srcline_off:{job_id}",
                   style="danger"))
    if can_restore:
        kb.add(Btn("🔗 Restore the source line", callback_data=f"job_srcline_on:{job_id}",
                   style="success"))
    kb.add(Btn(f"{G['back']}  Back to job", callback_data=f"queue_job:{job_id}", style="primary"))
    preview = base[:150] + ("…" if len(base) > 150 else "")
    safe_edit(call,
              f"🔗 <b>Source line for job #{job_id}</b>\n\n"
              f"Currently: <b>{state}</b>\n\n"
              f"📝 Description without it:\n<i>{escape(preview) or '(empty)'}</i>\n\n"
              "Stripping keeps the source link on the job — you can restore the "
              "line any time before the upload goes out.",
              kb)


@bot.callback_query_handler(func=lambda c: c.data.startswith("job_srcline_off:"))
def cb_job_srcline_off(call):
    uid = call.from_user.id
    job_id = int(call.data.split(":", 1)[1])
    if not get_job(job_id, uid):
        safe_answer(call, "Job not found.", alert=True)
        return
    result = toggle_job_source_line(job_id)
    safe_answer(call, "Source line stripped" if result == "off" else "Nothing to strip")
    cb_job_srcline(call)


@bot.callback_query_handler(func=lambda c: c.data.startswith("job_srcline_on:"))
def cb_job_srcline_on(call):
    uid = call.from_user.id
    job_id = int(call.data.split(":", 1)[1])
    if not get_job(job_id, uid):
        safe_answer(call, "Job not found.", alert=True)
    ...
    result = toggle_job_source_line(job_id)
    if result == "on":
        safe_answer(call, "Source line restored")
        cb_job_srcline(call)
    else:
        safe_answer(call, "No source link on this job, or your source line is off.", alert=True)
...
def cb_dl_queue(call):
    ...
    job_id = create_job(uid, default_channel_id(uid), title, file_path=path,
                        ...
                        description=build_import_description(
                            uid, title, "", row[4] or "", mark_source=True))
...
def start_link_download(uid: int, chat_id: int, url: str, edit_message: Optional[int] = None,
    ...
    def worker():
        ...
        # Build the description from the user's own templates. The source line
        # is stored inside a marked block so the job card can strip/restore it.
        creator = info.get('uploader') or 'unknown'
        description = build_import_description(uid, title, creator, url, mark_source=True)
        ...
        if (info.get("duration") or 999) <= 90:
            tags = source_hashtags or ",".join(t for t in ("viral", auto_tag) if t)
        else:
            tags = source_hashtags
    ...
    for index, item in enumerate(items, start=1):
        ...
        if not keep_only:
            ...
            jobs.append(create_job(uid, default_channel_id(uid), title, file_path=str(path),
                                   ...
                                   privacy=default_privacy(uid),
                                   description=build_import_description(
                                       uid, title, "", item["url"], mark_source=True)))
        try:
...
def tiktok_menu_text(uid: int) -> str:
    ...
    text += (f"\n✍️ Source line in descriptions: "
             f"<b>{'on' if user_source_line_template(uid) else 'off'}</b> "
             "(edit in Description &amp; credit)\n")
    return text
...
@bot.callback_query_handler(func=lambda c: c.data == "tk_credit")
def cb_tk_credit(call):
    uid = call.from_user.id
    safe_answer(call)
    safe_edit(call, tk_credit_text(uid), tk_credit_kb(uid))


@bot.callback_query_handler(func=lambda c: c.data == "tk_credit_toggle")
def cb_tk_credit_toggle(call):
    uid = call.from_user.id
    new_value = 0 if user_credit_source(uid) else 1
    save_user_credit_source(uid, new_value)
    audit(uid, "credit_source_set", str(new_value))
    safe_answer(call, f"Source line: {'on' if new_value else 'off'}")
    safe_edit(call, tk_credit_text(uid), tk_credit_kb(uid))
...
TK_TEMPLATE_FIELDS = {
    "tk_edit_desctpl": (
        "desc_template",
        "✍️ <b>Description template</b>\n\n"
        "This block is added at the top of every imported description.\n"
        "Placeholders: <code>{title}</code> <code>{creator}</code> <code>{source_url}</code>\n\n"
        "Send <code>off</code> to disable it. Current:\n<code>{current}</code>"),
    "tk_edit_creator": (
        "creator_line_template",
        "👤 <b>Creator line</b>\n\n"
        "One line crediting the original creator.\n"
        "Example: <code>Original creator: {{creator}}</code>\n\n"
        "Send <code>off</code> to remove this line. Current:\n<code>{current}</code>"),
    "tk_edit_srcline": (
        "source_line_template",
        "🔗 <b>Source line format</b>\n\n"
        "How the link to the original post is shown.\n"
        "Example: <code>Source: {{source_url}}</code> or <code>▶ {{source_url}}</code>\n\n"
        "Send <code>off</code> to remove this line. Current:\n<code>{current}</code>"),
    "tk_edit_shortstag": (
        "shorts_tag",
        "#️⃣ <b>Shorts auto-tag</b>\n\n"
        "This tag is added automatically to Short uploads (YouTube Shorts, "
        "TikTok auto-posts and short link imports).\n\n"
        "Send <code>off</code> to add no tag. Current:\n<code>{current}</code>"),
}


@bot.callback_query_handler(func=lambda c: c.data in (
        # Keep this literal tuple in sync with TK_TEMPLATE_FIELDS' keys:
        "tk_edit_desctpl", "tk_edit_creator", "tk_edit_srcline", "tk_edit_shortstag"))
def cb_tk_edit_template(call):
    """Ask for a new value for one of the configurable text fields."""
    uid = call.from_user.id
    safe_answer(call)
    field, intro = TK_TEMPLATE_FIELDS[call.data]
    current = tk_template_current(uid, field)
    set_state(uid, "tk_edit_template", field=field)
    safe_edit(call, intro.format(current=current or "(not set)"), back_kb("tk_credit"))


def tk_template_current(uid: int, field: str) -> str:
    if field == "desc_template":
        return user_desc_template(uid)
    if field == "creator_line_template":
        return user_creator_line_template(uid)
    if field == "source_line_template":
        return user_source_line_template(uid)
    return user_shorts_tag(uid)


def tk_template_save(uid: int, field: str, text: str) -> str:
    """Persist an edited template. Returns the stored value."""
    value = text.strip()[:600]
    if field == "desc_template":
        save_user_desc_template(uid, "" if value.lower() == "off" else value[:4800])
        return user_desc_template(uid)
    if field == "creator_line_template":
        save_user_creator_line_template(uid, value)
        return user_creator_line_template(uid)
    if field == "source_line_template":
        save_user_source_line_template(uid, value)
        return user_source_line_template(uid)
    save_user_shorts_tag(uid, "" if value.lower() == "off" else value)
    return user_shorts_tag(uid)


@bot.callback_query_handler(func=lambda c: c.data.startswith("tk_setcred:"))
def cb_tk_setcred(call):
    source_id = int(call.data.split(":", 1)[1])
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(Btn("🔗 On — include my source line", callback_data=f"tk_cred_{source_id}:1",
               style="success"))
    kb.add(Btn("🚫 Off — clean description", callback_data=f"tk_cred_{source_id}:0",
               style="danger"))
    kb.add(Btn("↩ Use my default", callback_data=f"tk_cred_{source_id}:d", style="primary"))
    kb.add(Btn(f"{G['no']}  Cancel", callback_data=f"tk_source:{source_id}", style="danger"))
    safe_answer(call)
    safe_edit(call, "🔗 <b>Put the TikTok link in the description?</b>\n\n"
                    "Off = description stays clean. '<i>Use my default</i>' follows the "
                    "main TikTok menu setting.", kb)


@bot.callback_query_handler(func=lambda c: c.data.startswith("tk_cred_"))
def cb_tk_cred_set(call):
    uid = call.from_user.id
    prefix, value = call.data.rsplit(":", 1)
    source_id = int(prefix.rsplit("_", 1)[1])
    if not get_source(source_id, uid):
        safe_answer(call, "Not found.", alert=True)
        return
    set_source_credit(source_id, uid, None if value == "d" else int(value))
    label = "default" if value == "d" else ("on" if value == "1" else "off")
    safe_answer(call, f"Source line: {label}")
    safe_edit(call, tiktok_menu_text(uid), tk_kb(uid))
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("tk_settitle:"))
def cb_tk_settitle(call):
    """Ask for a per-source title template."""
    uid = call.from_user.id
    source_id = int(call.data.split(":", 1)[1])
    if not get_source(source_id, uid):
        safe_answer(call, "Source not found.", alert=True)
        return
    src = get_source(source_id, uid)
    current = str(src[10] or "{title}")
    safe_answer(call)
    set_state(uid, "tk_edit_titletpl", source_id=source_id)
    safe_edit(call,
              "📝 <b>Title template for this source</b>\n\n"
              "Placeholders: <code>{title}</code> <code>{creator}</code> <code>{source_url}</code>\n\n"
              f"Current:\n<code>{escape(current[:200])}</code>\n\n"
              "<i>Send /cancel to abort.</i>", back_kb(f"tk_source:{source_id}"))
...
def admin_forcejoin_kb() -> types.InlineKeyboardMarkup:
    ...
    for chat_id, title, username, invite_link in channels:
...
def cb_adm_forcejoin(call):
    ...
    text = (f"🔒 <b>Force-Join Channels</b> ({len(channels)})\n\n"
            ...
            f"<b>Current requirements:</b>\n")
    if channels:
        for chat_id, title, username, invite_link in channels:
            verify = "✅" if chat_id and not str(chat_id).startswith("http") else "🤝"
            text += f"  {verify} {escape(title or username or chat_id)}\n"
            if invite_link:
                text += f"     🔗 {escape(invite_link[:50])}\n"
    else:
        text += "  <i>None — all users can access the bot</i>\n"
    text += ("\n<b>Verify modes:</b>\n"
             "  ✅ = bot checks membership (bot is admin)\n"
             "  🤝 = trust-based (user taps I Joined)\n\n"
             "<b>To add:</b> Send @username, invite link, or ID.")
...
def cb_adm_fj_add(call):
    ...
    safe_edit(call,
              ...
              "Send the channel info in any of these formats:\n\n"
              "<b>Public channel:</b>\n"
              "  <code>@channelname</code>\n\n"
              "<b>Numeric ID:</b>\n"
              "  <code>-1001234567890</code>\n\n"
              "<b>Invite link (private channels):</b>\n"
              "  <code>https://t.me/+abcDEFghiJk</code>\n\n"
              "<b>Full format (best for auto-verify):</b>\n"
              "  <code>Channel Name | https://t.me/+invite | -1001234567890</code>\n\n"
              "💡 For private channels, add the bot as admin for automatic "
              "membership checking. Without it, users just tap I Joined.",
...
def _ingest_telegram_video(uid: int, chat_id: int, file_id: str, file_size: int,
    ...
    def worker():
        ...
        if is_short:
            # Auto-add the user's Shorts tag (configurable; empty = add nothing)
            clear_state(uid)
            auto_tag = user_shorts_tag(uid)
            if auto_tag:
                short_tags = [t.strip() for t in tags.split(",") if t.strip()]
                if auto_tag.lower() not in [t.lower() for t in short_tags]:
                    short_tags.append(auto_tag)
...
def handle_text(message):
    ...
    # ── configurable text: description/creator/source templates, shorts tag ──
    if kind == "tk_edit_template":
        field = state.get("field", "")
        if field not in ("desc_template", "creator_line_template",
                         "source_line_template", "shorts_tag"):
            clear_state(uid)
            bot.reply_to(message, "❌ Unknown template field — try again from the menu.")
            return
        saved = tk_template_save(uid, field, text)
        clear_state(uid)
        labels = {
            "desc_template": "Description template",
            "creator_line_template": "Creator line",
            "source_line_template": "Source line",
            "shorts_tag": "Shorts auto-tag",
        }
        bot.reply_to(message,
                     f"✅ {labels[field]} saved."
                     + (f"\n<code>{escape(saved[:300])}</code>" if saved else "\n<i>(disabled)</i>"))
        safe_send(message.chat.id, tk_credit_text(uid), tk_credit_kb(uid))
        return

    # ── per-source TikTok title template ──
    if kind == "tk_edit_titletpl":
        source_id = int(state.get("source_id") or 0)
        clear_state(uid)
        if not get_source(source_id, uid):
            bot.reply_to(message, "❌ That source is gone.")
            return
        template = text.strip()[:200] or "{title}"
        db_query('UPDATE tiktok_sources SET title_template = ? WHERE source_id = ?',
                 (template, source_id))
        bot.reply_to(message, f"✅ Title template saved:\n<code>{escape(template)}</code>")
        return
    ...
    if kind == "adm_fj_add":
        ...
        invite_link = ""
        title = ""
        # Parse: "Channel Name | https://t.me/+invite | -1001234567890"
        if "|" in raw:
            ...
            for part in parts[1:]:
                ...
                if part.startswith("http"):
                    ...
                    chat_id = f"@{username}"
        elif raw.startswith("http") and "t.me" in raw:
            # Pure invite link
            invite_link = raw
            # Extract hash for display
            hash_match = re.search(r'\+([\w-]+)', raw)
            title = f"Channel +{hash_match.group(1)[:8]}" if hash_match else "Private Channel"
        elif raw.startswith("@"):
            username = raw.lstrip("@")
            chat_id = f"@{username}"
            title = username
            ...
            bot.reply_to(message,
                         ...
                         "  • <code>Name | link | ID</code>")
            return
        # Try to get channel info if we have a chat_id
        if chat_id:
            try:
                chat = bot.get_chat(chat_id)
                title = chat.title or title or chat_id
                if not username and chat.username:
                    username = chat.username
            except Exception:
                pass  # Can't access — still add with what we have
        # Determine the display label
        display_title = title or username or "Private Channel"
        # Save to database
        final_chat_id = chat_id or invite_link or title
        add_force_join_channel(final_chat_id, display_title, username, invite_link)
        audit(uid, "force_join_add", f"{display_title} ({final_chat_id})")
        clear_state(uid)
        # Build success message
        verify_mode = "✅ Hard check (bot is admin)" if chat_id else "🤝 Trust-based (user taps I Joined)"
        bot.reply_to(message,
                     f"✅ <b>Force-join channel added!</b>\n\n"
                     f"📺 {escape(display_title)}\n"
                     f"🔗 {escape(invite_link) if invite_link else '(no invite link)'}\n"
