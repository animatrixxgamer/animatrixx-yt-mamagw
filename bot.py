...
def diag_page():
    return page("Bot diagnostics",
                "<p>Bot is alive.</p>"
                f"<p>Token source: <code>{escape(value_source(BOT_TOKEN_ENV_KEY))}</code><br>"
...
def _handle_oauth_callback(state: str, code: str, error: str = ""):
    ...
    notify(uid,
           ...
           .add(Btn(f"{G['upload']}  Upload Something", callback_data="yt_upload_now", style="primary"))
           .add(Btn(f"{G['tiktok']}  Auto-post TikTok", callback_data="tk_menu", style="success")))
...
def main_menu_kb(uid: int) -> types.InlineKeyboardMarkup:
    ...
    # Row 3: Downloads + TikTok automation
    kb.add(
    ...
    # Row 4: Jobs / stats / saved files
    kb.add(
        Btn(f"{G['graph']}  My Uploads", callback_data="yt_jobs", style="primary"),
        Btn(f"{G['folder']}  My Downloads", callback_data="dl_list:0", style="primary"),
    )
    kb.add(Btn(f"{G['bolt']}  Bot Speed", callback_data="bot_speed", style="primary"))
    # Row 5: File hosting
    kb.add(
        Btn(f"{G['doc']}  Host File", callback_data="host_upload", style="primary"),
        Btn(f"{G['folder']}  My Files", callback_data="host_files", style="primary"),
    )
    # Row 6: Support
    kb.add(
...
HELP_TEXT = (
    ...
    "   • /tiktok @username for automatic TikTok → YouTube posting\n\n"
    "<b>3. Publish how you like</b>\n"
    "   • 🗓 Schedule: /schedule → pick a job → send <code>+2h</code> or <code>2026-09-20 18:30</code>\n"
    "   • ✏️ Edit title, description, tags, privacy on each job\n"
    "   • 📥 Everything waits in your Queue until you release it\n\n"
    "<b>Commands</b>\n"
    "/start · /menu — main menu\n"
    "/connect — connect a YouTube channel\n"
    "/upload — upload a video file\n"
    "/queue — pending &amp; scheduled uploads\n"
    "/schedule — schedule an upload\n"
...
def cmd_start(message):
    ...
    text = (
        ...
        f"<b>What I can do</b>\n"
        f"• 📤 Upload videos to YouTube with full metadata\n"
        f"• 🗓 Queue many videos and schedule each one\n"
        f"• 📥 Download from TikTok / Instagram / X / YouTube links\n"
        f"• 🎵 Mirror a TikTok account to YouTube automatically\n"
        f"• 🖥️ Host and run .py / .js scripts\n\n"
        f"👇 Pick an option, or send /help for the full guide."
    )
...
def prompt_for_video(chat_id: int, uid: int) -> None:
    ...
    safe_send(chat_id,
              "📤 <b>Send me videos</b>\n\n"
              "• Send one or many files — each becomes a job\n"
              "• Or paste a social link to download first\n"
              "• Add a caption to use as the title\n\n"
              "⚠️ Telegram only lets bots download up to 20MB, so for longer "
              "videos paste the link instead.",
              back_kb())
...
def cb_yt_select(call):
    ...
    safe_edit(call,
              f"✅ <b>{escape(channel_title(uid, channel_id))}</b> is now the active channel.\n\n"
              "New uploads, downloads and TikTok posts go here.",
              types.InlineKeyboardMarkup(row_width=1)
...
def cb_yt_upload(call):
    """Both the menu button and the deep link land here."""
    uid = call.from_user.id
    safe_answer(call)
    if is_banned(uid):
        safe_answer(call, "You are blocked from using this bot.", alert=True)
    ...
    if not user_channels(uid):
        safe_edit(call,
                  ...
                  .add(Btn(f"{G['back']}  Back", callback_data="main_menu", style="danger")))
        return
    prompt_for_video(call.message.chat.id, uid)
    ...
    if kind == "yt_paste_code":
        ...
        try:
            bot.edit_message_text(
                ...
                f"\U0001f194 <code>{escape(info['channel_id'])}</code>",
                message.chat.id, status_msg.message_id,
                reply_markup=types.InlineKeyboardMarkup(row_width=1)
                .add(Btn(f"{G['upload']}  Upload Something", callback_data="yt_upload_now", style="primary"))
                .add(Btn(f"{G['tiktok']}  Auto-post TikTok", callback_data="tk_menu", style="success")),
