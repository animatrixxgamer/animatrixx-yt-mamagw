...
The startup banner now answers this for you instead of leaving you to guess:
it prints `💾 Persistent volume: <name> — /data survives redeploys` when storage
is really on the volume, and a `⚠️ NOT persistent` block that names the two
files you are about to lose (`bot_data.db` + `secret.key`) when it is not. If a
volume is attached but `STORAGE_DIR` still points at `/app/storage`, the banner
prints the exact value to set.

**2. ~~The root `Dockerfile` builds the wrong app.~~ — fixed.** It only copied
`src/`, never `bot.py`, and ran `uvicorn src.main:app` (async aiogram + Postgres
+ Redis). Railway prefers a root Dockerfile over auto-detection, so a deploy
could build an image in which `bot.py` does not exist. That Dockerfile, and the
duplicate app it built, have both been deleted — nothing in the repo can hijack
the build now, and nothing else can poll Telegram with the same `BOT_TOKEN`.
`railway.toml` still pins `builder = "NIXPACKS"` and `startCommand = "python
bot.py"`. Worth one look in Railway → the service → Settings → Build to confirm
the Builder reads "Nixpacks" rather than a stale "Dockerfile Path".
