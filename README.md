...
- **Source-credit line** — by default mirrored descriptions end with
  `Source: <tiktok link>`. Everything the bot appends to descriptions is
  editable under **TikTok → Description & credit**: the source line format,
  the creator-credit line, a custom description template (with
  `{title}`, `{creator}`, `{source_url}` placeholders) and the Shorts
  auto-tag. Send `off` as a value to disable any line, or flip the
  source line on/off per source in its settings
- **Per-job source line toggle** — every queued job card has a
  `Source line: on/off` button: strip the credit right before uploading,
  restore it any time before the upload goes out
