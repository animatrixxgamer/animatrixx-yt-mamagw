...
> **Two entry points live in this repo**
>
> | File | What it is | Run it with |
> |------|------------|-------------|
> | `bot.py` | **The deployed bot.** Standalone, SQLite, colored buttons, all features below. `Procfile` → `python bot.py` | `python bot.py` |
> | `src/` | A separate FastAPI + aiogram + Postgres/Celery rewrite (not the deployment target) | `uvicorn src.main:app` |

Everything below describes `bot.py`.
