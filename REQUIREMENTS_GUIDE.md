...
8. Add **Authorized redirect URIs**: `http://localhost:8080/oauth/youtube/callback` plus one for each host you deploy
to (`https://<your-service>.up.railway.app/oauth/youtube/callback`). Each must
match `YOUTUBE_REDIRECT_URI` byte for byte, or Google answers HTTP 400 on the
token exchange.
